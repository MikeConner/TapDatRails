module NfcTagsHelper
  def add_payload(tag, form_builder, content_type)
    ...
    form_builder.fields_for :payloads, tag.payloads.build(:threshold => 1), :child_index => 'NEW_RECORD' do |payload_form|
      html = render(:partial => 'payload', :locals => { :f => payload_form, :content_type => content_type })
      onclick = "$('#{escape_javascript(html)}'.replace(/NEW_RECORD/g, $('.payload').length)).insertBefore('#yapa_insertion_point'); return false;"

      content_tag(:a, "Add #{content_type} Yapa", :href => '#', :onclick => onclick, :id => 'add_payload')
    end
  end
end
