class CurrenciesController < ApplicationController
  ...
  def leader2
    ...
    x=1
    @tapperlist = Array.new
    @tappers.each do |tapper|
      @tapperlist << {:rank => x, :name => tapper.user.name, :total => tapper.total, :taps => tapper.taps, :image => tapper.user.mobile_profile_thumb_url || tapper.user.profile_image_url(:thumb).to_s}
      x=x+1
    end
    x=1
